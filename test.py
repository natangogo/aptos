...
def test_process(model, pth_path_first, device):
    ...
    # バッチなしで1フレームずつ読み込む
    for frame, label, timestamp, video_id in tqdm.tqdm(test_loader):
        if video_id not in video_dict:
            video_dict[video_id] = {"frames": [], "labels": [], "timestamps": []}
        video_dict[video_id]["frames"].append(frame)
    ...
    with torch.no_grad():
        for video_id, data in video_dict.items():
            ...
            # 推論（GL_MSTCNのforward）
            outputs = model(video[0], frame_num=frame_num)  # [T, num_classes]
