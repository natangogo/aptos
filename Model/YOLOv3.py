...
class YOLOv3(nn.Module):
    def __init__(self, 
        ...
        self.block_A1 = Block_A(channels*32) # 入力: 1024 → 出力: 512
        self.block_B1 = Block_B(in_channels=channels*16, out_channels=(num_classes+5)*3)
        # self.yololayer
        self.block_C1 = Block_C(channels*16)# 入力: 1024 → 出力: 512

        self.block_A2 = Block_A(channels*8 + channels*16)# 入力: 1024 → 出力: 256
        self.block_B2 = Block_B(in_channels=(channels*8 + channels*16) // 2, out_channels=(num_classes+5)*3)
        # self.yololayer
        self.block_C2 = Block_C((channels*8 + channels*16) // 2)# 入力: 512 → 出力: 256

        self.block_A3 = Block_A(channels*8 + channels*4)# 入力: 512 → 出力: 128
        self.block_B3 = Block_B(in_channels=(channels*8 + channels*4) // 2, out_channels=(num_classes+5)*3)
    
    def forward(self, x):
        x_res8, x_res8_2, x_res4 = self.darknet53(x)
        x_a = self.block_A1(x_res4)
        y1 = self.block_B1(x_a) #13x14x255
        x_c = self.block_C1(x_a)
        x_out1 = torch.cat([x_res8_2, x_c], dim=1)

        x_a2 = self.block_A2(x_out1)
        y2 = self.block_B2(x_a2) #26x26x255
        x_c2 = self.block_C2(x_out1)
        x_out2 = torch.cat([x_res8, x_c2], dim=1)

        x_a3 = self.block_A3(x_out2)
        y3 = self.block_B3(x_a3) #52x52x255
