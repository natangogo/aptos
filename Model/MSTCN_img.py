...
class Separable_Conv(nn.Module):
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(in_channels, in_channels, kernel_size, dilation=1, padding=(kernel_size-1)//2*1, groups=in_channels)
        self.conv2 = nn.Conv2d(in_channels, in_channels, kernel_size, dilation=2, padding=(kernel_size-1)//2*2, groups=in_channels)
        self.conv3 = nn.Conv2d(in_channels, in_channels, kernel_size, dilation=4, padding=(kernel_size-1)//2*4, groups=in_channels)
        self.conv4 = nn.Conv2d(in_channels, in_channels, kernel_size, dilation=5, padding=(kernel_size-1)//2*5, groups=in_channels)
        self.conv_second1 = nn.Conv2d(in_channels, out_channels, kernel_size=1)
        self.conv_second2 = nn.Conv2d(in_channels, out_channels, kernel_size=1) 
        self.conv_second3 = nn.Conv2d(in_channels, out_channels, kernel_size=1) 
        self.conv_second4 = nn.Conv2d(in_channels, out_channels, kernel_size=1) 
...
class MSD_Block(nn.Module):
    def __init__(self,
        ...
        self.average_pool = nn.AvgPool2d(kernel_size=3, stride=3)
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=1)
        self.norm = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU() 
