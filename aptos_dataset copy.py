            ...
            try:
                ...
                while second < duration:
                    try:
                        ...
                        # frame = reader[frame_index]  # numpy array (H, W, 3), dtype=uint8
                        # timestamp = reader.get_frame_timestamp(frame_index)  # 秒単位
                        timestamp_val = round(reader.get_frame_timestamp(frame_index)[0], 2)
                        label = self._get_label_for_timestamp(timestamp_val, annots)
                        # print(f"[DEBUG] Checking frame_index={frame_index}, timestamp={timestamp_val:.2f}")
                        
                        if label is None:
                            t += fps_step
                            print(f"[No Match] t={timestamp_val:.2f} not in any: " +
                                    ", ".join(f"[{s:.2f}-{e:.2f}]" for (s, e, _) in annots))
                            continue
                        
                        # Tensor に変換して transform
                        frame = reader[frame_index]
                        frame = torch.from_numpy(frame.asnumpy()).permute(2, 0, 1)  # (C, H, W)
                        frame = self.transform(frame)

                        yield frame, label, float(timestamp_val), vid, float(frame_rate)

                    except Exception as e:
                        # print(f"[ERROR] frame_index={frame_index}, err={e}")
                        continue  # スキップ
                    second += fps_step
